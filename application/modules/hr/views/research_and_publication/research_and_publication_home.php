    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="pageheader" style="background-color: #F5F5F5;">
                <h2><i class="fa fa-home"></i>Employee Research and publication page <br></h2>
                <ol class="breadcrumb" style="background-color: #F5F5F5;">
                    <li><a href="<?php echo base_url() ?>hr/research_and_publication/all_research_and_publication">Dashboard</a></li>
...
<script>
    function add_research_and_publication() {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>hr/research_and_publication/load_add_research_and_publication_from',
            success: function (result) {
                if (result) {
                    $('#load_from').html(result);
                    $('#load_title').html("Add research and publication");
                    return false;
                } else {
                    return false;
                }
...
<script>
    function update_research_and_publication($id) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>hr/research_and_publication/load_update_research_and_publication_from/' + $id,
            success: function (result) {
                if (result) {
                    $('#load_from').html(result);
                    $('#load_title').html("Update award/honours Info");
                    return false;
                } else {
                    return false;
                }
...
<script>
    function details_research_and_publication($id) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>hr/research_and_publication/load_details_research_and_publication_from/' + $id,
            success: function (result) {
                if (result) {
                    $('#load_from').html(result);
                    $('#load_title').html("Details research and publication");
                    return false;
                } else {
                    return false;
                }
...
<script>
    function search_content() {
        ...
        if ($.trim(emp_id) != "") {
            $.ajax({
                type: 'post',
                url: '<?php echo base_url(); ?>hr/research_and_publication/all_research_and_publication',
                data: {emp_id: emp_id},
            ...
            $.post(base_url + "hr/research_and_publication/all_research_and_publication/", function (data) {
                $("#ajaxdata").html(data);
            });
        }
    }
