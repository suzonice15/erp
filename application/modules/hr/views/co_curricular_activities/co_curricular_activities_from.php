<form action="#" id="add_co_curricular_activities" method="post" enctype="multipart/form-data">
...
<script>
    $(document).ready(function () {
        $('#add_co_curricular_activities').submit(function () {
            var value = CKEDITOR.instances['description'].getData();
            var dataString = new FormData($(this)[0]);
            dataString.append("description", value);
            $.ajax({
                type: "POST",
                url: '<?php echo base_url() ?>hr/co_curricular_activities/create_co_curricular_activities',
                data: dataString,
                async: false,
                success: function (result) {
                    if (result) {
                        ...
                        $('#add_co_curricular_activities').trigger("reset");
                        window.setTimeout(function () {
                            window.location.href = "<?php echo base_url()?>hr/co_curricular_activities/all_co_curricular_activities";
                        }, 1000);
                        return false;
                    } else {
                        return false;
