<form action="#" id="add_research_and_publication_frm" method="post">
...
<script>
    $(document).ready(function () {
        $("#add_research_and_publication_frm").submit(function () {
            var dataString = $('#add_research_and_publication_frm').serialize();
            $.ajax({
                type: "POST",
                url: '<?php echo base_url() ?>hr/my_resume/create_research_and_publication',
                data: dataString,
                success: function (result) {
                    if (result) {
                        $('#add_msg').html(result);
                        $('#add_msg .alert').delay(5000).fadeOut(1000);
                        $('#add_research_and_publication_frm').trigger("reset");
