<form action="#" id="add_training_frm" method="post">
...
<script>
    $(document).ready(function () {
        $("#add_training_frm").submit(function () {
            var dataString = $('#add_training_frm').serialize();
            $.ajax({
                type: "POST",
                url: '<?php echo base_url() ?>hr/my_resume/create_training',
                data: dataString,
                success: function (result) {
                    if (result) {
                        $('#add_msg').html(result);
                        $('#add_msg .alert').delay(5000).fadeOut(1000);
                        $('#add_training_frm').trigger("reset");
