        ...
        <div class="col-lg-12">
            <div class="pageheader" style="background-color: #F5F5F5;">
                ...
                <ol class="breadcrumb" style="background-color: #F5F5F5;">
                    <li><a href="<?php echo base_url() ?>hr/reference/all_reference">Dashboard</a></li>
                                        ...
                                        foreach ($reference as $v_data) {
                                            ...
                                            <tr>
                                                ...
                                                <td style="width: 110px;">
                                                    <a href="#"
                                                       onclick="details_reference(<?php echo $v_data->emp_id; ?>)"
                                                    ...
                                                    <a href="#"
                                                       onclick="update_reference(<?php echo $v_data->emp_id; ?>)"
                                                       class="btn btn-success" data-backdrop="static"
                                                       data-keyboard="false" data-toggle="modal"
                                                       data-target="#load_modal"
                                                       id="update_reference"
...
<script>
    function add_reference() {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>hr/reference/load_add_reference_from',
            success: function (result) {
                if (result) {
                    $('#load_from').html(result);
                    $('#load_title').html("Add Employee Reference Info");
                    return false;
                } else {
                    return false;
                }
...
<script>
    function update_reference($id) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>hr/reference/load_update_reference_from/' + $id,
            success: function (result) {
                if (result) {
                    $('#load_from').html(result);
...
<script>
    function details_reference($id) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>hr/reference/load_details_reference_from/' + $id,
            success: function (result) {
                if (result) {
                    $('#load_from').html(result);
                    $('#load_title').html("Details Reference Info");
                    return false;
                } else {
                    return false;
                }
...
<script>
    function search_content() {
        ...
        if ($.trim(emp_id) != "") {
            $.ajax({
                type: 'post',
                url: '<?php echo base_url(); ?>hr/reference/all_reference',
                data: {emp_id: emp_id},
            ...
            $.post(base_url + "hr/reference/all_reference/", function (data) {
                $("#ajaxdata").html(data);
            });
        }
    }
