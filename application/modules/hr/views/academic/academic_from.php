<form action="" id="add_academic" method="post">
    <table class="add_more_field table table-bordered table-striped table-hover">
        <tr>
            <td colspan="4">Employee ID</td>
            <td colspan="5">
                <input id="emp_id" name="emp_id" type="text" class="form-control" placeholder="Employee ID">
            </td>
        </tr>
        <tr>
...
<script>
    $(document).ready(function () {
        $("#add_academic").submit(function () {
            var dataString = $('#add_academic').serialize();
            $.ajax({
                type: "POST",
                url: '<?php echo base_url() ?>hr/academic/create_academic',
                data: dataString,
                success: function (result) {
                    if (result) {
                        $('#add_msg').html(result);
                        $('#add_msg .alert').delay(5000).fadeOut(1000);
                        $('#add_academic').trigger("reset");
                        window.setTimeout(function () {
                            window.location.href = "<?php echo base_url()?>hr/academic/all_academic";
                        }, 1000);
                        return false;
                    } else {
                        return false;
