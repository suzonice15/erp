    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="pageheader" style="background-color: #F5F5F5;">
                <h2><i class="fa fa-home"></i>Employee Weekend Info Page <br></h2>
                <ol class="breadcrumb" style="background-color: #F5F5F5;">
                    <li><a href="<?php echo base_url() ?>hr/weekend/all_weekend">Dashboard</a></li>
                ...
                <div class="panel-heading" style="height: 55px;">
                    <div class="col-md-2">
                        <button type="button" class="btn btn-success" data-backdrop="static"
                                ...
                                data-target="#load_modal" onclick="return add_weekend();">
                            Add Weekend
                        </button>
                    </div>
                    <div class="col-md-2">
                                        ...
                                        foreach ($weekend as $v_data) {
                                            ...
                                            <tr>
                                                ...
                                                <td style="width: 70px;">
                                                    <?php if ($v_data->status == 1) {?>
                                                        <a href="#"
                                                           onclick="update_weekend(<?php echo $v_data->id; ?>)"
...
<script>
    function add_weekend() {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>hr/weekend/load_add_weekend_from',
            success: function (result) {
                if (result) {
                    $('#add_load_from').html(result);
                    $('#add_load_title').html("Add Employee Weekend Info");
                    return false;
                } else {
                    return false;
                }
...
<script>
    function update_weekend($id) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>hr/weekend/load_update_weekend_from/' + $id,
            success: function (result) {
                if (result) {
...
<script>
    function search_content() {
        ...
        if ($.trim(emp_id) != "" || (exam_name_id) != "" || (board_id) != "" || (study_group_id) != "") {
            $.ajax({
                type: 'post',
                url: '<?php echo base_url(); ?>hr/weekend/all_weekend',
            ...
            $.post(base_url + "hr/weekend/all_weekend/", function (data) {
                $("#ajaxdata").html(data);
            });
        }
    }
