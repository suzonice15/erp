    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="pageheader" style="background-color: #F5F5F5;">
                <h2><i class="fa fa-home"></i>Employee Job Posting Info Page <br></h2>
                <ol class="breadcrumb" style="background-color: #F5F5F5;">
                    <li><a href="<?php echo base_url() ?>hr/job_posting/all_job_posting">Dashboard</a></li>
        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading" style="height: 55px;">
                    <div class="col-md-2">
                        <button type="button" class="btn btn-success" id="add_job_posting" data-backdrop="static"
                                data-keyboard="false" data-toggle="modal"
                                data-target="#load_modal" onclick="return add_job_posting();">
                            Add job Posting
                        </button>
                    </div>
                    <div class="col-md-3">
...
<script>
    function add_job_posting() {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>hr/job_posting/load_add_job_posting_from',
            success: function (result) {
                if (result) {
                    $('#load_from').html(result);
                    $('#load_title').html("Add Employee Job Posting Info");
                    return false;
                } else {
                    return false;
                }
...
<script>
    function update_job_posting(id) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>hr/job_posting/load_update_job_posting_from/' + id,
            success: function (result) {
                if (result) {
                    $('#load_from').html(result);
                    $('#load_title').html("Update Employee Job Posting Info");
                    return false;
                } else {
                    return false;
                }
...
<script>
    function search_content() {
        ...
        if ($.trim(emp_id) != "" || (department_id) != "") {
            $.ajax({
                type: 'post',
                url: '<?php echo base_url(); ?>hr/job_posting/all_job_posting',
            ...
            $.post(base_url + "hr/job_posting/all_job_posting/", function (data) {
                $("#ajaxdata").html(data);
            });
        }
    }
    ...
    function main_page_pagination($this) {
        ...
        if (url != '') {
            $.ajax({
                type: "POST",
                url: url,
                data: {emp_id: emp_id, department_id: search_department_id},
