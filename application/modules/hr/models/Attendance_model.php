...
class Attendance_Model extends CI_Model
{
    ...
    {
        $this->db->select('count(hr_attendance.emp_id) count_total_rows');
        $this->db->where('hr_job.emp_id = hr_attendance.emp_id');
        $this->db->where('hr_job.status', 1);
        $this->db->like($field, $cond, 'after');
        $query_result = $this->db->get('hr_attendance,hr_job');
    ...
    public function select_attendance($limit, $start)
    {
        $this->db->limit($limit, $start);
        $this->db->select('hr_attendance.*,master_department.department_name,master_shift.shift_name,master_section.section_name');
        $this->db->where('hr_job.emp_id = hr_attendance.emp_id');
        $this->db->where('hr_job.status', 1);
        $this->db->where('hr_attendance.emp_id = hr_job_posting.emp_id');
        ...
        $query_result = $this->db->get('hr_attendance,hr_job,hr_job_posting');
