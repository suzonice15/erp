        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading" style="height: 55px;">
                    <div class="col-md-2">
                        <button type="button" class="btn btn-success" id="add_class_subject" data-backdrop="static"
                                data-keyboard="false" data-toggle="modal"
                                data-target="#load_modal" onclick="return add_class_subject();">
                            Add Class Subject
                        </button>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        <select onchange="return search_content();" class="form-control" id="search_subject_id"
                                name="search_subject_id">
                            <option value="0">Search by subject name</option>
                                        ...
                                        foreach ($class_subject as $v_data) {
                                            ...
                                            <tr>
                                                ...
                                                <td style="width: 145px;">
                                                    <a href="#"
                                                       onclick="update_class_subject(<?php echo $v_data->id; ?>)"
                                                       class="btn btn-success" data-backdrop="static"
                                                       data-keyboard="false" data-toggle="modal"
                                                       data-target="#load_modal"
                                                       id="update_class_subject"
...
<script>
    function add_class_subject() {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>master/class_subject/load_add_class_subject_from',
            success: function (result) {
                if (result) {
                    $('#load_from').html(result);
                    $('#load_title').html("Add class subject");
                    return false;
                } else {
                    return false;
                }
...
<script>
    function update_class_subject(id) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>master/class_subject/load_update_class_subject_from/' + id,
            success: function (result) {
                if (result) {
                    $('#load_from').html(result);
                    $('#load_title').html("Update class subject");
                    return false;
                } else {
                    return false;
                }
...
<script>
    function search_content() {
        ...
        if ($.trim(class_id) != "" || (subject_id) != "") {
            $.ajax({
                type: 'post',
                url: '<?php echo base_url(); ?>master/class_subject/all_class_subject',
            ...
            $.post(base_url + "master/class_subject/all_class_subject/", function (data) {
                $("#ajaxdata").html(data);
            });
        }
    }
