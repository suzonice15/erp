    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="pageheader" style="background-color: #F5F5F5;">
                <h2><i class="fa fa-home"></i>Admission Fee Page <br></h2>
                                        ...
                                        foreach ($admission_fee as $v_data) {
                                            ...
                                            <tr>
                                                ...
                                                <td style="width: 110px;">
                                                    ...
                                                    if ($v_data->status == 1) { ?>
                                                        <a href="#"
                                                           onclick="admission_fee_inactive(<?php echo $v_data->id; ?>)"
                                                           class="btn btn-success" title="Inactive">
                                                            <i class="glyphicon glyphicon-arrow-down"></i>
                                                        </a>
                                                    <?php } else { ?>
                                                        <a href="#"
                                                           onclick="admission_fee_active(<?php echo $v_data->id; ?>)"
...
<script>
    function add_admission_fee() {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>master/admission_fee/load_add_admission_fee_from',
...
<script>
    function update_admission_fee(id) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>master/admission_fee/load_update_admission_fee_from/' + id,
            success: function (result) {
                if (result) {
                    $('#load_from').html(result);
...
<script>
    function admission_fee_active($id) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>master/admission_fee/admission_fee_active/' + $id,
            success: function (result) {
                if (result) {
...
<script>
    function admission_fee_inactive($id) {
        $.ajax({
            type: "POST",
            url: '<?php echo base_url() ?>master/admission_fee/admission_fee_inactive/' + $id,
            success: function (result) {
                if (result) {
